...
<div class="container-fluid d-flex justify-content-between w-50 my-5">
    <form class="needs-validation me-5" novalidate name="registrationForm" action="controller"
          method="post">
        ...
        <div class="mb-3">
            <div class="form-outline">
                ...
                <select class="form-select" id="barber_select" name="barber" required>
                    <option selected disabled value=""><fmt:message key="order.barber.option"
                                                                    bundle="${rb}"/></option>
    ...
    <form class="needs-validation ms-5" novalidate name="registrationForm" action="controller"
          method="post">
        <input type="hidden" name="command" value="go_to_booked_slot">
        <div class="mb-3">
            <div class="form-outline">
                <label for="date_free" class="form-label"><fmt:message key="order.time_booked" bundle="${rb}"/></label>
                <input type="date" id="date_free" class="form-control" name="dataSelect"/><br>
                <div class="invalid-feedback"><fmt:message key="order.data" bundle="${rb}"/></div>
            </div>
        </div>
        <button type="submit" class="btn btn-success mt-2 mb-3" value="Order">
            <fmt:message key="order.button" bundle="${rb}"/>
        </button>
    </form>
</div>
