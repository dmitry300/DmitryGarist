...
public class ListOfActiveBarbersCommand implements Command {
    private static final Logger logg = LogManager.getLogger(ListOfActiveBarbersCommand.class);
    ...
    public Router executeCommand(HttpServletRequest request) {
        ...
        List<Barber> barbers;
        try {
            barbers = serviceFactory.getBarberService().findActiveBarbers();
